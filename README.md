...
An AI-powered automated video audio dubbing pipeline running on Hugging Face Spaces with Gradio SDK and ZeroGPU support.

## 🚀 Architecture
1. **Downloader**: Audio extraction with `yt-dlp` (16kHz mono WAV).
2. **ASR Transcriber**: Timestamped English transcription with `faster-whisper` (ZeroGPU accelerated).
3. **Director**: Translation and timing coordination with Gemini Multi-Model Cascade (`gemini-1.5-flash` ➔ `gemini-2.0-flash` ➔ `gemini-1.5-pro`) via the official `google-genai` SDK.
